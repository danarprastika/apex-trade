...
from app.api.deps import get_current_user, get_execution_service, get_paper_trading_service, get_risk_service, get_signal_service, get_strategy_service
from app.core.exceptions import PermissionDeniedError
from app.database.models.identity import User
from app.schemas.trading import OrderCreate, OrderRead, PaperOrderCreate, PaperOrderRead, SignalCreate, SignalRead, StrategyCreate, StrategyRead
...
def list_orders(
    ...
    return execution_service.list_orders(user_id=current_user.id)
...
def create_order(
    ...
    if risk_score is None:
        risk_score = signal_service.get(payload.signal_id).confidence if payload.signal_id else 100.0
    requested_position_size = payload.quantity * (payload.price or 0)
    decision = risk_service.evaluate(current_user.id, risk_score, requested_position_size)
    if not decision.allowed:
        raise PermissionDeniedError(f"Risk veto: {', '.join(decision.veto_reasons)}")
    return execution_service.create_order(
...
def create_paper_order(
    ...
    if current_user.role not in {"TRADER", "ADMIN", "SUPER_ADMIN"}:
        raise PermissionDeniedError("Permission denied")
    result = paper_trading_service.execute_order(
        user_id=current_user.id,
